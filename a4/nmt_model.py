...
class NMT(nn.Module):
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        Y_split = torch.split(Y, split_size_or_sections=len(Y), dim=0)
        for counter1 in range(len(Y_split[0])):
            #rint(Y_split[0][counter1].shape)
            temp_var = torch.squeeze(Y_split[0][counter1])
            y_bart = torch.cat((temp_var, o_prev), dim = 1)
            #print("THIS RAN")
            temp_return = self.step(y_bart, dec_state, enc_hiddens, enc_hiddens_proj, enc_masks)
            decod_next = temp_return[0]
            o_new = temp_return[1]
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        batch_size_here = len(new_dec_hidden)
        multi_arr = []
        for counter1 in range((batch_size_here)):
            src_len_tensor = enc_hiddens_proj[counter1]
            dec_hidden_tensor = new_dec_hidden[counter1]
            multi_ = torch.matmul(src_len_tensor, dec_hidden_tensor)
            multi_arr.append(multi_)
        e_t = torch.stack(multi_arr)
        ### END YOUR CODE
        ...
        softmax_stuff = nn.Softmax(dim=1)
        alpha_t = softmax_stuff(e_t)
        #alpha_t = nn.Softmax(e_t, 1)
        temp_arr = []
        for counter1 in range(len(alpha_t)):
            temp_arr.append(torch.matmul(torch.transpose(enc_hiddens[counter1], 0, 1), alpha_t[counter1]))
        a_t = torch.stack(temp_arr)
        U_t = torch.cat((a_t, new_dec_hidden), 1)
